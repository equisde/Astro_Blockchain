...
// Copyright (c) 2017 The Astro developers
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
		...
		throw runtime_error(
			...
			"  start        - Start masternode configured in astro.conf\n"
			"  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
			"  start-many   - Start all masternodes configured in masternode.conf\n"
			"  stop         - Stop masternode configured in astro.conf\n"
			...
			//"  vote-many    - Vote on a Astro initiative\n"
			//"  vote         - Vote on a Astro initiative\n"
            );
    if (strCommand == "stop")
    {
