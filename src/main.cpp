...
const string strMessageMagic = "Astro Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		...
		nSubsidy = 3 * COIN;
	else if (pindexBest->nHeight <= 2000000) // Block 2m ~ 4m ASTRO
		nSubsidy = 4 * COIN;
	else if (pindexBest->nHeight <= 3000000) // Block 3m ~ 3m ASTRO
		nSubsidy = 3 * COIN;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in ASTRO.
    // So much so, People wanted to bring ASTRO to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of ASTRO
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s ASTRO (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s ASTRO (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Astro: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Astro", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
	//printf("---------------------------------------------------------------------------------------------------\n");
	//printf(uint256("0xdc7fbc8f99362bf75e301048634039f8ab870dad40c186d90c5f91227dfa0934").ToString().c_str());
        //printf("\n");
	//printf(block.hashMerkleRoot.ToString().c_str());
        //printf("\n---------------------------------------------------------------------------------------------------\n");
	//printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0xde4931963770ea52bf5d7747b401476ca804b8be126707addd3a64e2ab708990"));
        block.print();
