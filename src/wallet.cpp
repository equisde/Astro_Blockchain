...
int64_t CWallet::GetAnonDebit(const CTxIn& txin) const
{
    ...
    // -- amount of owned astro decreased
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != GetMNCollateral()*COIN); // do not use MN funds 5,000 ASTRO
                } else {
                    found = true;
                }
                if(!found) continue;
...
// astro: total coins available for staking - WIP needs updating
        ...
        if(!masternodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee)){
            int winningNode = GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    ...
                    else burnAddress = "ASTROXXXXXXXXXXXXXXXXXXXXXXXXXZeeDTw";
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s ASTRO %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s ASTRO %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::SendDnrToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (vNodes.empty())
    {
        sError = _("Error: Astro is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    {
        sError = "Insufficient Anonymous ASTRO funds";
        return false;
    };
