...
const QString BaseURL = "http://astro.io/ASTROusd.php";
const QString BaseURL2 = "http://astro.io/ASTRObtc.php";
double astrox;
double ASTRObtcx;
...
if (what == BaseURL) // Astro Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString astro = finished->readAll();
    astrox = (astro.toDouble());
    astro = QString::number(astrox, 'f', 2);

	dollarg = astro;
}
if (what == BaseURL2) // Astro BTC Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString ASTRObtc = finished->readAll();
    ASTRObtcx = (ASTRObtc.toDouble());
    ASTRObtc = QString::number(ASTRObtcx, 'f', 8);
