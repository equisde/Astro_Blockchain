...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 5000 ASTRO at the same time
};
...
public:
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nASTRO, int64_t& nAnon, const isminefilter& filter) const
    {
        nASTRO = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                nASTRO += GetCredit(txout, filter);
            if (!MoneyRange(nASTRO)
                || !MoneyRange(nAnon))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
...
public:
    ...
    mutable int64_t nCredASTROCached;
    ...
    bool GetCredit(int64_t& nCredASTRO, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredASTRO = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredASTROCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredASTROCached, nCredAnonCached, ISMINE_SPENDABLE);
            fCreditSplitCached = true;
        };

        nCredASTRO = nCredASTROCached;
