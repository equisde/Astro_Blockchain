...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop Astro server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "Astro server stopping, please wait a few minutes for full shutdown...";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "sendASTROtoanon",          &sendASTROtoanon,          false,  false},
    { "sendanontoanon",         &sendanontoanon,         false,  false},
    { "sendanontoASTRO",          &sendanontoASTRO,          false,  false},
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: astro-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use astrod";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n %s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Astro Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "sendASTROtoanon"          && n > 1) ConvertTo<double>(params[1]);
    if (strMethod == "sendanontoanon"         && n > 1) ConvertTo<double>(params[1]);
    if (strMethod == "sendanontoASTRO"          && n > 1) ConvertTo<double>(params[1]);
